...
// تمیز کردن
// متد متد کردن اضافات و فراخوانی آنها در بلوک های 
// if and else 

    void MainMenu()
    {
            string UserInput = Chap_help();
    ...
    string Chap_help()
    {
        ...
        writeln("this software Sopport of Windows and Linux (I test On Void Linux)");
        writeln("for Use windows , you can type :" , "windows" , "for linux you can type " , "linux" , GLVclass.tRESET);
        while(true)
        {
            string inputUser = readln.strip().toLower();
            if(inputUser == "windows")
            {
                return "windows";
            }else if(inputUser == "linux")
            {
                return "linux";
            }else
            {
                writeln("please enter  : (windows) , (linux)");
            }
        }
